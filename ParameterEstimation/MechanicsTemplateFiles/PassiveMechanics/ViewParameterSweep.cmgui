...
for ($i=0;$i<=$tot;$i=$i+1)
{
	gfx read node;ParameterSweep_ED/LVInflationED_$i time $t;
	$t ++;
}
gfx read element;ParameterSweep_ED/LVInflationED_0;
...
for ($i=0;$i<=$tot;$i=$i+1)
{
	gfx read data;OptimisedExfile/Surface_epi_1 time $t region Surface_Points_Epi ;
	$t ++;
...
for ($i=0;$i<=$tot;$i=$i+1)
{
	gfx read data;OptimisedExfile/Surface_endo_1 time $t region Surface_Points_Endo;
	$t ++;
}
...
for ($i=0;$i<=$tot;$i=$i+1)
{
	gfx read data;ParameterSweep_ED/EndoError_$i time $t region EndoError;
	$t ++;
}
...
for ($i=0;$i<=$tot;$i=$i+1)
{
	gfx read data;ParameterSweep_ED/EpiError_$i time $t region EpiError;
	$t ++;
