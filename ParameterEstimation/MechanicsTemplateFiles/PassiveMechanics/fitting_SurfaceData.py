...
import fitting_tools_dataonly

# ==================================================================#
def readSurfaceData(fileName):
    try:
        fitDataAnisotropicScaleRigidNoCorr_ModelTree_TwoSurfacesfile = open(fileName, 'r')
    except IOError:
...
def readFibreIpdata(fileName):
    ...
    for l in lines:
        coords.append([float(string) for string in l.split()[1:4]])
        fibres.append([float(string) for string in l.split()[4:7]])
    coords = array(coords)
    fibres = array(fibres)

    return coords, fibres
...
def fitDataAnisotropicScaleRigidNoCorr_ModelTree_TwoSurfaces(X1, X2, data1, data2, filename, xtol=1e-5, maxfev=0,
    ...
    print 'Error of registration: ' + str(err)
...
#==========================================================================#
#==========================================================================#
def writeFibreIpdata(d, v, filenamew):
    """ Write the newly registered fibre data into ipdata file. Each row
    in d is [x,y,z] of a data point, and the remaining three numbers in
    the row are the components of the fibre vector. Read the transformed fibre
    vector file to get the fibre vectors """
...
#==========================================================================#
def writeTransformation_Scaling_Translation(t, filename):
