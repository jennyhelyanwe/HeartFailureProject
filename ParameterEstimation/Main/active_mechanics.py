...
def active_get_ED(study_id):
    ...
    copy('WarmStartSolution/CurrentInflated_1.ipinit',
         '../ActiveMechanics/WarmStartSolution/CurrentContracted_1.ipinit')
...
def active_init_TCa(pressure, prev_idx, cur_idx):
    ...
    if float(TCa_prev) == 0.0:
        TCa = (pressure[cur_idx-1]-pressure[prev_idx-1])*5.0
    elif cur_idx < 10:
        TCa = float(TCa_prev)*scale*1.2
    else:
        TCa = float(TCa_prev)*scale
    print 'LOG_STATUS: Initial guess for TCa @ frame '+str(cur_idx)+' is '+str(TCa)+'\n'
    return TCa
    ...
    if CMISS_monitor==1:
        ...
        while converged == 0:
            ...
            if converged == 0:
                TCa_step *= 0.5
                if TCa_step < 5e-2:
                    print 'LOG_CURRENT: Something has gone wrong at (warm solve) frame '+str(frame_num)+'... TCa_step = ' +str(TCa_step) + '\n'
                    quit()
                num_steps = abs(math.ceil(TCa_increm/TCa_step))
...
def active_bounds_get(pressure,  prev_idx, cur_idx):
    ...
    f.close()
    if p > 0:
        lb = float(TCa_cur)*0.75
    else:
        lb = float(TCa_cur)*0.3
    if cur_idx == 2:
        lb = float(TCa_cur)*0.8
    ub = float(TCa_cur)*2
    if ub < 20:
        ub = 20
    print 'LOG_STATUS: The bounds for frame '+str(cur_idx)+' are '+str(lb)+' and '+str(ub)+'\n'
    return lb, ub
...
def active_sensitivity_evaluate(study_id, study_frame):
    ...
    print 'LOG: Begin evaluating TCa sensitivity at each optimised frame...'
    os.chdir(os.environ['STUDIES']+study_id+'/LVMechanics'+study_id+'/ActiveMechanics')
    ...
    for i in range(1, len(idx)):
        ...
        if min(test) > 0:
            ...
            # Get geometric data
            data_epi = os.environ['GEOM_DATA']+study_id+'/Active/'+study_id+'_Surface_Points_Epi_'\
                       + str(idx[i+1])+'.ipdata'
            data_endo = os.environ['GEOM_DATA']+study_id+'/Active/'+study_id+'_Surface_Points_Endo_'\
                        + str(idx[i+1])+'.ipdata'
            copy(data_epi, 'Surface_Points_Epi_ES.ipdata')
            copy(data_endo, 'Surface_Points_Endo_ES.ipdata')
            TCa_apex = 0.0
            ...
            for j in range(0, len(h)):
                print 'Current TCa = '+str(test[j])
                time.sleep(5)
                activation_create_ipacti(test[j], TCa_apex, 'TCa_ZERO_exclude_apex.ipacti', 'TCa_current.ipacti')
