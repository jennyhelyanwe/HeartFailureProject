...
if isexist==0
    cd('../../STF_Images/');
    study_name=input('Please enter the name of the image study .....\n','s');
    No_Frame=input('Please enter the total number of frames for the current study .....\n');
    current_study=ImageFile(25:29);
    [R_R_MRI]=ExtractTriggerTime(study_name,No_Frame,current_study);
...
CIM_Volume_File=['../ParamEstimation/CIMModels/',study_name,'/volumes/info/',study_name,'_model_ca.model_',study_name,'_AY'];
fidr=fopen(CIM_Volume_File,'r');
%% Initialise the line being read
line_read=0;
volume_data=fgets(fidr);
...
title(['Pressure Volume Loop ',study_name],'FontSize',12);
